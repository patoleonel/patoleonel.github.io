...
const i18n = {
    en: {
        heroTitle: "Welcome to my journey",
        heroSubtitle: "Below, you can explore my interconnected academic and professional paths.",
        colAcad: "Academic Milestones",
        colProf: "Professional Milestones",
        colTake: "Key Takeaways",
        navVideo: "INTRO VIDEO",
        navSpiderweb: "MY JOURNEY",
        navProjects: "PROJECTS",
        navSidequests: "SIDE QUESTS",
        titleProjects: "Projects & Works",
        titleSidequests: "Side Quests",
        sq1: "Golf handicap 13",
        sq2: "Quantum Learner",
        sq3: "EFSET C1 Advanced Certificate",
        sq4: "PGA Golf Instructor Assistant Certificate",
        certLink: "View Official Certificate ↗",
        videoPlaceholder: "Coming soon: English presentation video link"
    },
    es: {
        heroTitle: "Bienvenido a mi camino",
        heroSubtitle: "A continuación, puedes explorar mis caminos interconectados a nivel académico y profesional.",
        colAcad: "Hitos Académicos",
        colProf: "Hitos Profesionales",
        colTake: "Aprendizajes Clave",
        navVideo: "INTRO VIDEO",
        navSpiderweb: "MI CAMINO",
        ...
        sq4: "Certificado Auxiliar de Profesor de Golf (PGA)",
...
const connectionObserver = new ResizeObserver(() => {
    if (drawAnimationFrame) cancelAnimationFrame(drawAnimationFrame);
    drawAnimationFrame = requestAnimationFrame(drawConnections);
});
...
function setLanguage(lang) {
    ...
    document.getElementById('hero-subtitle').innerText = i18n[lang].heroSubtitle;
    ...
    document.getElementById('nav-sidequests').innerText = i18n[lang].navSidequests;
    document.getElementById('title-projects').innerText = i18n[lang].titleProjects;
    ...
    document.getElementById('sq-4-text').innerText = i18n[lang].sq4;
...
function createNodeElement(item, type) {
    const div = document.createElement('div');
    div.className = `milestone-node ${type}-node`;
    div.setAttribute('data-id', item.id);
    div.setAttribute('data-type', type);
    if (item.connections) {
        div.setAttribute('data-connections', JSON.stringify(item.connections));
    }
    ...
    if (type === 'takeaway') {
        ...
        div.innerHTML = `
            ...
            <h3 class="node-title">${item['title_' + currentLang]}</h3>
            <p class="node-desc">${item['description_' + currentLang]}</p>
            ${quantumGallery}
        `;
    }
...
function renderTimeline() {
    ...
    const grid = document.getElementById('timeline-grid-nodes');
    grid.innerHTML = '';

    domNodes.academic = {};
    domNodes.professional = {};
    domNodes.takeaways = {};

    const maxLen = Math.max(
        portfolioData.academicMilestones.length,
        portfolioData.professionalMilestones.length,
        portfolioData.keyTakeaways.length
    );

    for (let i = 0; i < maxLen; i++) {
        // 1. Academic
        if (portfolioData.academicMilestones[i]) {
            const acadItem = portfolioData.academicMilestones[i];
            const acadNode = createNodeElement(acadItem, 'academic');
            grid.appendChild(acadNode);
            domNodes.academic[acadItem.id] = { elem: acadNode, data: acadItem };
            connectionObserver.observe(acadNode);
        } else {
            grid.appendChild(document.createElement('div'));
        }

        // 2. Takeaway
        if (portfolioData.keyTakeaways[i]) {
            const takeItem = portfolioData.keyTakeaways[i];
            const takeNode = createNodeElement(takeItem, 'takeaway');
            grid.appendChild(takeNode);
            domNodes.takeaways[takeItem.id] = { elem: takeNode, data: takeItem };
            connectionObserver.observe(takeNode);
        } else {
            grid.appendChild(document.createElement('div'));
        }

        // 3. Professional
        if (portfolioData.professionalMilestones[i]) {
            const profItem = portfolioData.professionalMilestones[i];
            const profNode = createNodeElement(profItem, 'professional');
            grid.appendChild(profNode);
            domNodes.professional[profItem.id] = { elem: profNode, data: profItem };
            connectionObserver.observe(profNode);
        } else {
            grid.appendChild(document.createElement('div'));
        }
    }
...
function drawConnections() {
    ...
    const getCoords = (elem) => {
        ...
        return {
            ...
            leftEdge: rect.left - svgRect.left,
            rightEdge: rect.right - svgRect.left,
            topEdge: rect.top - svgRect.top,
            bottomEdge: rect.bottom - svgRect.top
        };
    };
    ...
    // Draw lines for academic (Left to Center)
    Object.values(domNodes.academic).forEach(nodeObj => {
        ...
        data.connections.forEach(targetId => {
            if (domNodes.takeaways[targetId]) {
                ...
                // End at the left edge of the takeaway node
                const end = { x: endCoords.leftEdge, y: endCoords.y };
                createLine(svg, start, end, data.id, targetId, false);
            }
        });
    });

    // Draw lines for professional (Right to Center)
    Object.values(domNodes.professional).forEach(nodeObj => {
        ...
        data.connections.forEach(targetId => {
            if (domNodes.takeaways[targetId]) {
                const endCoords = getCoords(domNodes.takeaways[targetId].elem);
                // End at the right edge of the takeaway node
                const end = { x: endCoords.rightEdge, y: endCoords.y };
                createLine(svg, start, end, data.id, targetId, false);
            }
        });
    });
    ...
    nodes.forEach(node => {
        ...
        node.addEventListener('mouseenter', (e) => {
            ...
            lines.forEach(line => {
                ...
                if (source === nodeId || target === nodeId || connections.includes(target) || connections.includes(source)) {
            ...
            // Highlight connected takeaway nodes (scale them up)
            connections.forEach(connId => {
                const targetNode = document.querySelector(`.takeaway-node[data-id="${connId}"]`);
                if (targetNode) {
                    targetNode.classList.add('highlight-node');
                }
            });

            // Bidirectional: if hovering a takeaway, highlight the milestones that point to it
            if (node.classList.contains('takeaway-node')) {
                document.querySelectorAll('.milestone-node:not(.takeaway-node)').forEach(mNode => {
                    const mConns = JSON.parse(mNode.getAttribute('data-connections') || '[]');
                    if (mConns.includes(nodeId)) {
                        mNode.style.transform = 'scale(1.05)';
                        mNode.style.borderColor = 'var(--text-light)';
                    }
                });
            }
        });
        ...
        node.addEventListener('mouseleave', () => {
            // Remove line highlights and restore opacity
            ...
            // Reset bidirectional highlights
            if (node.classList.contains('takeaway-node')) {
                document.querySelectorAll('.milestone-node:not(.takeaway-node)').forEach(mNode => {
                    mNode.style.transform = '';
                    mNode.style.borderColor = '';
                });
            }
        });
    });
